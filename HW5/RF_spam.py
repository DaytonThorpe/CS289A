...
class DecisionTree():
    ...
    def segmenter(self,set):
        ...
        #print random_features[np.argsort(random_features)]
...
num_samples = len(features_shuffled) #1000
...
max_depth = 35
min_size = 4
num_rand_features = int(np.floor(np.sqrt(num_features)))
#num_rand_features = num_features / 2
#print int(np.ceil(np.sqrt(num_features)))
#num_rand_features = 5
...
for i in range(len(test_features)):
    ...
    counts = np.bincount(candidate_label)
    predicted_label = np.argmax(counts)
    string = str(i) + "," + str(predicted_label)
