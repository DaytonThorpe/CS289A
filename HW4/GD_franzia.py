...
features_kaggle = franzia_train_raw['X_test']
...
#add the ones to Kaggle set
new_features = np.zeros([len(features_kaggle),d+1])
new_features[:,:-1] = features_kaggle
new_features[:,-1] = 1.0
features_kaggle = new_features
...
for i in range(d-1): #don't center the bias feature
    ...
    for j in range(len(features_kaggle)): features_kaggle[j][i] -= mean
...
for i in range(d-1): #don't normalize the bias feature
    ...
    for j in range(len(features_kaggle)): features_kaggle[j][i] /= np.sqrt(var)
...
for i in range(100):
    ...
    grad = -np.transpose(features_shuffled).dot(labels_shuffled - s)
...
#make predictions
n = len(features_kaggle)

print "Id,Category"
for i in range(n):
    s = 1 / (1 + np.exp(-features_kaggle[i].dot(w)))
    if s >= 0.5: string = str(i) + "," + str(1)
    else: string = str(i) + "," + str(0)
